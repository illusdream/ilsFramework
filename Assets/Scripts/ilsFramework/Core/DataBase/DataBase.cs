...
namespace ilsFramework.Core
{
    ...
    public static class DataBase
    {
        ...
        {
#if UNITY_EDITOR
            if (!Directory.Exists($"{Application.streamingAssetsPath}/DataBases")) Directory.CreateDirectory($"{Application.streamingAssetsPath}/DataBases");
#endif
            var sb = new StringBuilder($"{Application.streamingAssetsPath}/DataBases/");
            if (folders.Length > 0)
        ...
        {
            if (!Directory.Exists($"{Application.persistentDataPath}/DataBases")) Directory.CreateDirectory($"{Application.persistentDataPath}/DataBases");
            var sb = new StringBuilder($"{Application.persistentDataPath}/DataBases/");
            if (folders.Length > 0)
                for (var i = 0; i < folders.Length; i++)
                {
                    sb.Append(folders[i]);
        ...
        {
            var cDestDbPath = copyMode switch
            {
            ...
            var cSrcDbPath = copyMode switch
            {
            ...
            if (!File.Exists(cSrcDbPath)) throw new FileNotFoundException();

            var destDirPath = Path.GetDirectoryName(cDestDbPath);
            if (!Directory.Exists(destDirPath) && destDirPath != null) Directory.CreateDirectory(destDirPath);
