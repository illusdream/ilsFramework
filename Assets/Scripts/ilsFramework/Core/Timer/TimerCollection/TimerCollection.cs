...
namespace ilsFramework.Core
{
    public class TimerCollection
    {
        ...
        public Timer this[string timerName]
        {
            get
            {
                if (_timers.TryGetValue(timerName, out var timer)) return timer;
                return null;
            }
        ...
        {
            if (_timers.TryGetValue(timerName, out var timer)) TimerManager.Instance.RemoveTimer(timer);
        }
        ...
        {
            foreach (var timer in _timers.Values) TimerManager.Instance.RemoveTimer(timer);
            _timers.Clear();
        }
    }
}