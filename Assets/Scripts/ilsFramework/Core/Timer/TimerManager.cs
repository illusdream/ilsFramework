...
namespace ilsFramework.Core
{
    public partial class TimerManager : ManagerSingleton<TimerManager>, IManager
    {
        ...
        public Timer RegisterTimer(float cycleTime, int executingTimes, float delayTime = 0, bool isFrameTimer = false, Action<Timer> onStart = null,
            Action<Timer> onCompleted = null, Action<Timer> onFinish = null, Action<Timer> onCycling = null)
        {
            var id = timerIDCounter;
            timerIDCounter++;
            var result = new Timer(id, cycleTime, delayTime, executingTimes, isFrameTimer, onStart, onCompleted, onFinish, onCycling);
        ...
        /// <param name="onFinish">完成全部计时时要做什么</param>
        /// <param name="onCycling">在计时过程中的每一帧要做什么</param>
        public void GetOrResetTimer(ref Timer timer, float cycleTime, int executingTimes, float delayTime = 0, bool isFrameTimer = false,
            Action<Timer> onStart = null, Action<Timer> onCompleted = null, Action<Timer> onFinish = null, Action<Timer> onCycling = null)
        {
            //取消掉前一个计时器
