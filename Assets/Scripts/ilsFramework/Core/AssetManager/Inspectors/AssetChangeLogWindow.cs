...
namespace ilsFramework.Core
{
    public class AssetChangeLogWindow : OdinEditorWindow
    {
        ...
        private List<AssetChangeType> GetAllMatchOptions()
        {
            ...
            foreach (var _enum in Enum.GetValues(typeof(AssetChangeType))) list.Add((AssetChangeType)_enum);
            return list;
        }
        ...
        private void UpdateSearch()
        {
            ...
            var set = MatchOption.ToHashSet();
            var assetDataBaseConnection = DataBase.GetStreamingConnection(AssetManager.AssetDataBasePath);

            if (!assetDataBaseConnection.TryGetTable<AssetChangeLog>(out var table)) assetDataBaseConnection.CreateTable<AssetChangeLog>();

            var list = assetDataBaseConnection.Table<AssetChangeLog>().Where(a => set.Contains(a.ChangeType)).ToList();
            AssetChangeLogInfos = new List<AssetChangeLogeDetailWindow>();
            var connection = DataBase.GetStreamingConnection(AssetManager.AssetDataBasePath);
