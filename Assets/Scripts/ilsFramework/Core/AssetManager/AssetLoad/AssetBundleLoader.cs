...
namespace ilsFramework.Core
{
    ...
    {
        //资源缓存与引用
        [ShowInInspector] private Dictionary<string, AssetBundle> _loadedAssetBundles = new();

        private AssetBundle _mainAB;

        [ShowInInspector] private AssetBundleManifest _mainManifest;

        private bool _mainManifestLoaded;

        [ShowInInspector] private Dictionary<string, int> _referenceCounts = new();
        ...
        public AssetBundle LoadAssetBundle(string bundleName)
        {
            ...
            {
                if (!_loadedAssetBundles.ContainsKey(dependency)) LoadAssetBundle(dependency);
        ...
        public void UnLoadUnUsedAssetBundle(bool unloadAllLoadedObjects)
        {
            ...
            foreach (var key in needRemove) UnloadAssetBundle(key, unloadAllLoadedObjects);
        }
        ...
        private IEnumerator IE_LoadAssetBundleAsync(string bundleName, Action callback)
        {
            if (!_loadedAssetBundles.ContainsKey(bundleName))
            {
                foreach (var dependency in _mainManifest.GetAllDependencies(bundleName))
                {
                    if (!_loadedAssetBundles.ContainsKey(bundleName)) yield return IE_LoadAssetBundleAsync(dependency, null);
                    _referenceCounts[dependency]++;
                }
        ...
        private IEnumerator IE_LoadAssetAsync(AssetBundle assetBundle, string assetName, Type type, Action<Object> callback)
        {
            ...
            yield return abr;
            if (abr.asset) callback?.Invoke(abr.asset);
        }
