...
namespace ilsFramework.Core
{
    public class AssetChangeLog
    {
        ...
        {
            var changes = new List<AssetChangeLog>();
            var _needCheck = needCheck == null ? new HashSet<AssetChangeType>() : needCheck.ToHashSet();

            if (old == null || newValue == null) return changes;

            var old_sp = old.AssetKey.Split('.');
            var new_sp = newValue.AssetKey.Split('.');
