...
namespace ilsFramework.Core
{
    ...
    public class FileTrieNode<T>
    {
        ...
        [ShowInInspector] public Dictionary<string, FileTrieNode<T>> Children;
        ...
        [ShowInInspector] public string Path => GetPath();
        ...
        {
            //存在该子节点
            if (Children.TryGetValue(fileIEnumerator.CurrentFileName, out var fileTreeNode))
            {
            ...
            {
                var nodeType = fileIEnumerator.IsEnd() ? EFileTrieNodeType.Asset : EFileTrieNodeType.Folder;
                var nodeValue = fileIEnumerator.IsEnd() ? value : defaultConstructor.Invoke();
                ...
                var nodePath = fileIEnumerator.CurrentFileName;
                if (fileIEnumerator.MoveNext()) Children[nodePath].AddChild(fileIEnumerator, value, defaultConstructor);
            }
        }
        ...
        public bool TryGet(FileIEnumerator fileIEnumerator, out T value)
        {
            if (Children.TryGetValue(fileIEnumerator.CurrentFileName, out var fileTreeNode))
            {
                if (fileIEnumerator.MoveNext()) return fileTreeNode.TryGet(fileIEnumerator, out value);
        ...
        public bool Contains(FileIEnumerator fileIEnumerator)
        {
            if (Children.TryGetValue(fileIEnumerator.CurrentFileName, out var fileTreeNode))
            {
                if (fileIEnumerator.MoveNext()) return fileTreeNode.Contains(fileIEnumerator);
