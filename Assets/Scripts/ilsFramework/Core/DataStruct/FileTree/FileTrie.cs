...
namespace ilsFramework.Core
{
    ...
    {
        [ShowInInspector] public FileTrieNode<T> root;

        private Func<T> defaultConstructor;

        public FileTrie(string RootFolderName, T RootObject, Func<T> DefaultConstructor)
        {
            root = new FileTrieNode<T>(EFileTrieNodeType.Folder, RootFolderName, RootObject);
        ...
        {
            if (fileIEnumerator.CurrentFileName == root.Name && fileIEnumerator.MoveNext()) return root.Contains(fileIEnumerator);
            return false;
        }
        ...
        private class FileTrieEnumerator : IEnumerator<FileTrieNode<T>>
        {
            ...
            {
                if (stack.Count == 1 && stack.Peek().Children.Count == 0) return false;

                var cNode = stack.Pop();
                foreach (var child in cNode.Children.Values.Reverse()) stack.Push(child);
                return true;
            }
            ...
            public FileTrieNode<T> Current
            {
                get
                {
                    if (stack == null) throw new InvalidOperationException("未初始化迭代器");
