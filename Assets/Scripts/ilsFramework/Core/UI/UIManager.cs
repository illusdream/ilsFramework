...
namespace ilsFramework.Core
{
    public class UIManager : ManagerSingleton<UIManager>, IAssemblyForeach
    {
        ...
        {
            var type = typeof(T);
            if (uiPanels.TryGetValue(type, out var value)) return value as T;
            ...
            if (uiPanelSettings.TryGetValue(type, out var setting))
            {
                ...
                if (uiPanelGameObject)
                {
                    ...
                    foreach (var field in needFields)
                    {
                        ...
                        if (field.Item2.TargetComponentPath == "") childObject = goInstance.transform;
                        ...
                        if (childObject && childObject.gameObject.TryGetComponent(field.Item1.FieldType, out var elementInstance)) uiElement = elementInstance;
                    ...
                    {
                        if (property.Item1.GetSetMethod(true) == null) continue;
                        object uiElement = null;
        ...
        {
            if (uiPanels.TryGetValue(typeof(T), out var result) && result is T panel) return panel;
        ...
        {
            if (uiPanels.ContainsKey(typeof(T))) needRemoveFromDic.Remove(typeof(T));
        }
    }
}