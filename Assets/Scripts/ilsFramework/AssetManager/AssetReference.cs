...
namespace ilsFramework
{
    ...
    public class AssetReference<T> where T : UnityEngine.Object
    {
        ...
        [ShowInInspector]
        [SerializeField]
        private EAssetLoadMode loadMode;
        [ShowInInspector]
        [SerializeField]
        ...
        private void AssetChangeListener()
        {
            ...
            if (assetDataBaseConnection.TryGetTable<AssetInfo>(out var table))
            {
                ...
                else
                {
                    ...
                    //resource
                    if (ilsFramework.Asset.CheckAssetInResourcesFolder(path))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            throw new ArgumentException($"{path}该资源不在Resourece文件夹内且不属于任意的AssetBundle，请分配资源存储方式");
                        }
